<template>
	<view class="content">
		<view class="header_info">
			<image src="../../static/my/mine_bg.jpg" mode="aspectFill"></image>
		...
		<view class="member-card">
			<view class="member-head">
				<view class="user-pic">
					<view class="pic-box"><open-data type="userAvatarUrl"></open-data></view>
					<view class="user-label" v-if="member">
						<image
							:src="'../../static/my/level' + levelCurrtab + '.png'"
							mode="aspectFill"
						></image>
						<!-- :src="currentLev.background" -->
				...
				<view class="user-detail" v-if="member">
					...
					<view class="card-no">卡号：{{ memberinfo.cardNo || '' }}</view>
					<view class="slider-box">
						<view class="slider"><sildermine :config="sliderConfig"></sildermine></view>
						<view class="fl">
							{{ `${memberinfo.experience ? memberinfo.experience : 0}` }}/{{
								`${currentLev.upperLimit ? currentLev.upperLimit : 0}`
							}}
						</view>
					</view>
					<view
						class="upgrade_juide"
						v-if="currentLev.upperLimit - memberinfo.experience < 0"
					>
						恭喜您，已经达到最高等级
					</view>
					<view class="upgrade_juide" v-else>
						在累计{{
							currentLev.upperLimit - memberinfo.experience || 0
						}}经验值就可以升级
					</view>
				</view>
		...
		<view class="min-service-box">
			<view class="head-tit">我的服务</view>
			<view class="service-box">
				<view
					class="service-item"
					v-for="(item, index) in servicearr"
					:key="index"
					@click="jumpClassIfyPage(index)"
				>
...
import { subtr,accAdd } from '../../utils/utils.js';
export default {
	data() {
		return {
			notAuth: false,
	...
	methods: {
		...
		async getGradeInfo() {
			...
			if (res.code == 200) {
				...
				let lev = res.data.filter((item, index) => {
					if (
						(experience == item.lowerLimit || experience > item.lowerLimit) &&
						(experience < item.upperLimit || experience == item.lowerLimit)
					) {
						this.levelCurrtab = accAdd(index + 1);
						console.log('当前等级索引为',this.levelCurrtab)
						return item;
					}
				});
				if (lev.length == 0) {
