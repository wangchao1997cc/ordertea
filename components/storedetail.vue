<template>
	...
	<view class="store_detail" :style="{height:shopBoxHeight + 'rpx'}" :hidden="showdetail">
		...
		<view class="store_detail_title">
			...
			<text>距门店{{shopinfo.deliveryRanges[0].radius?shopinfo.deliveryRanges[0].radius:'0'}}m范围内起送\n</text>
			<text>(配送范围受地区、天气等多方因素影响，以实际可配送距离为准)\n</text>
		</view> -->
		<view class="store_detail_title">
			<text class="detail_title">门店信息\n\n</text>
			<view class="Btime">营业时间：
				<!-- <block v-for="(item,index) in shopinfo.strBusinessTime" :key="index"> -->
					<text>{{shopinfo.strBusinessTime}}</text>
				<!-- </block> -->
			</view>
			<text @click="callTel(shopinfo.strTelephoneNum)">门店电话：{{shopinfo.strTelephoneNum}}\n</text>
			<text>门店地址：{{shopinfo.strAddress}}</text>
		</view>
